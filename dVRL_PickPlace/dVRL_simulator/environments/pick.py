...
class PSMPickEnv(PSMEnv_Position):#, utils.EzPickle):
    def __init__(self, psm_num = 1, reward_type='sparse', randomize_obj = True, randomize_ee = False):
        ...
        super(PSMPickEnv, self).__init__(psm_num = psm_num, n_substeps = 1, block_gripper = False,
                        ...
                        target_offset = [0,0,0.038], obj_range = 0.025, target_range = 0.075,
                        distance_threshold = 0.003, initial_pos = initial_pos, reward_type = reward_type,
                        dynamics_enabled = True, two_dimension_only = False, 
                        randomize_initial_pos_obj = randomize_obj, randomize_initial_pos_ee = randomize_ee,
			randomize_initial_or_obj = True, randomize_initial_or_kidney = True,
			randomize_initial_pos_kidney = True, docker_container = "vrep_ee_pickplace")
